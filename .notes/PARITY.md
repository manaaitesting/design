...
- **Motion — 2026-08-30.** Run on a branch *parallel* to the third sweep, not
  after it, which is why both called themselves the sixth pass; they are one
  pass each on a different surface, merged here. Figma Motion: a timeline on a
  frame, not a transition between two. Fifteen rows first (`MO-01`…`MO-15`: 11
  parity, 2 partial, 2 missing), then the four that were not `parity` worked one
  at a time plus a bug sweep, which added `MO-16` and `MO-17` for behaviour the
